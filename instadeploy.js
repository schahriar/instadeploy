...
InstaDeploy.prototype.watch = function(directoryPath, remotePath) {
	...
	Watcher(directoryPath, remotePath, context.options.ignore || ['.git/**', 'node_modules/**'], function FILE_ON_CHANGE(PATH, directory, remote) {
		...
		context.smartQueueList.push({ localPath: PATH, remotePath: path.join(remote, path.relative(directory, PATH)), relativePath: path.relative(directory, PATH), callback: function(error) {
			...
			if(!error) context.emit('uploaded', path.relative(directory, PATH), PATH, path.join(remote, path.relative(directory, PATH)));
			else context.emit('failed', error, path.relative(directory, PATH), PATH, path.join(remote, path.relative(directory, PATH)));
		}});
